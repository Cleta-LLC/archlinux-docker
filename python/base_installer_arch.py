...
def install_base(host, username, port, password, root_password="root"):
    """This script will setup su, pacman, sudo, groups, newuser."""
...
def setup_user(host, username, port, password, root_password="root"):
    """Script that activates a new user after being created."""
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    try:
        ssh.connect(host, port, username, password)
...
if __name__ == '__main__':
    ...
    #root_password = ""
    # setup_user(host, user, port, password, root_password)
    # setup_yay()